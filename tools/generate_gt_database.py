...
class GTDatabaseGenerator(KittiDataset):
    ...
    def generate_gt_database(self):
        gt_database = []
        for idx, sample_id in tqdm(enumerate(self.image_idx_list), total=len(self.image_idx_list)):
            # sample_id = int(sample_id)
            # print('process gt sample (id=%06d)' % sample_id)
            # print('process gt sample (token={})'.format(sample_id))

            # pts_lidar = self.get_lidar(sample_id)
            pts_lidar = self.get_lidar(idx)
