...
class KittiDataset(torch_data.Dataset):
    def __init__(self, root_dir, split='train'):
        ...
        self.image_idx_list = list(sorted([x.strip() for x in open(split_dir).readlines()]))
    ...
    def get_image(self, idx):
        ...
        img_file = os.path.join(self.image_dir, '{}.png'.format(self.image_idx_list[idx]))
    ...
    def get_lidar(self, idx):
        lidar_file = os.path.join(self.lidar_dir, '{}.bin'.format(self.image_idx_list[idx]))
        assert os.path.exists(lidar_file)
        return np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)

    def get_calib(self, idx):
        calib_file = os.path.join(self.calib_dir, '{}.txt'.format(self.image_idx_list[idx]))
    ...
    def get_road_plane(self, idx):
        plane_file = os.path.join(self.plane_dir, '{}.txt'.format(self.image_idx_list[idx]))
